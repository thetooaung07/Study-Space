		...
		const fetchData = async () => {
			...
			try {
				const details = await api.get<any>(
					`/groups/${groupId}/details` + (user ? `?requestingUserId=${user.id}` : ""),
				);
										...
										<>
											<Link href={`/groups/${groupId}/analytics`}>
												<Button
													variant="outline"
													size="icon"
													className=" bg-white/40 backdrop-blur-sm border-white/50"
													title="Analytics"
												>
				...
				onSuccess={(session) => {
					if (session?.id) {
						...
						const fetchSessions = async () => {
							try {
								const sessionsData = await api.get<StudySessionDTO[]>(
									`/sessions/group/${groupId}` + (user ? `?requestingUserId=${user.id}` : ""),
								);
			...
			<AlertDialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							This action cannot be undone. This will permanently delete the group and remove all data
							associated with it.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
