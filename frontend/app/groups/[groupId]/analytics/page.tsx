...
const LeaderboardItem = ({ member, index, maxMinutes, isCurrentUser }: Readonly<LeaderboardItemProps>) => {
	// NOSONAR
	const rank = index + 1;
	const avgMinutes = member.sessionCount > 0 ? Math.round(member.totalMinutes / member.sessionCount) : 0;
		...
		>
			<div className="flex items-center justify-between mb-3">
				<div className="flex items-center gap-4 flex-1">
					{/* Rank Badge */}
					<div
						className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg ${getRankBadgeClass(rank)}`}
					>
						{getRankIcon(rank)}
					</div>
			...
			<div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
				<div
					...
					className={`h-full transition-all ${getProgressBarClass(rank)}`}
				/>
			</div>
...
export default function GroupAnalyticsPage({ params }: Readonly<{ params: Promise<{ groupId: string }> }>) {
					...
					<div className="flex items-center gap-2">
						<Button
							...
							className={
								dateRange === "thisWeek"
									? ""
									: "backdrop-blur-md bg-white/60 border-white/40 hover:bg-secondary"
							} // NOSONAR
						>
							This Week
						</Button>
