...
export const workspacesApi = {
  /** Get my workspaces */
  getMyWorkspaces: (userId: number): Promise<StudentWorkspace[]> =>
    api.get(`/workspaces/my?userId=${userId}`),

  /** Get workspace detail */
  getById: (id: number): Promise<StudentWorkspace> =>
  ...
  delete: (id: number, userId: number): Promise<void> =>
    api.delete(`/workspaces/${id}?userId=${userId}`),

  // ─── Spaces ──────────────────────────────────────────────────────────────

  getSpaces: (workspaceId: number): Promise<WorkspaceSpace[]> =>
  ...
  forkCourse: async (workspaceId: number, userId: number, courseId: number, title?: string): Promise<WorkspaceSpace> => {
  ...
  getSpace: (spaceId: number): Promise<WorkspaceSpace> =>
    api.get(`/workspaces/spaces/${spaceId}`),
  ...
  updateSection: (sectionId: number, userId: number, data: CreateWorkspaceSectionRequest): Promise<WorkspaceSection> =>
    api.put(`/workspaces/sections/${sectionId}?userId=${userId}`, data),
  ...
  uploadMaterial: async (
    ...
    const res = await fetch(
      `${BASE_URL}/workspaces/sections/${sectionId}/materials?userId=${userId}`,
      {
        method: "POST",
...
export const contributionsApi = {
  ...
  ): Promise<ContributionProposal> => {
    const token = localStorage.getItem("token");
    const res = await fetch(
      `${BASE_URL}/contributions/${proposalId}/review?userId=${userId}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
