...
function TimeDisplay({ hours, minutes, seconds }: Readonly<TimeDisplayProps>) {
	return (
		<div className="flex items-baseline justify-center font-mono leading-none">
			...
			<span className="text-4xl font-medium text-muted-foreground/50 ml-2 tabular-nums">:{seconds}</span>
		</div>
	);
}
...
export function TimerCard() {
	...
	return (
		<Card className="relative p-8 bg-gradient-to-br from-primary/10 to-secondary/10 border-primary/20 overflow-hidden">
			<ModeSelector mode={mode} isSessionActive={isSessionActive} onModeChange={handleModeChange} />
			...
			<div className="text-center space-y-6 mt-4">
				...
				<div className="flex items-center justify-center gap-2 text-muted-foreground/80">
					...
					<h3 className="text-lg font-medium">{mode === "stopwatch" ? "Elapsed Time" : "Time Remaining"}</h3>
				</div>
