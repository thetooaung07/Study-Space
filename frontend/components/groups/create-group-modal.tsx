			...
			<DialogContent className="max-w-md p-0 overflow-hidden shadow-none border-none">
				<Card className="p-6 w-full border-none shadow-xl">
					...
					<div className="space-y-5">
						<div>
							<label htmlFor="group-name" className="text-sm font-medium text-foreground block mb-2">Group Name</label>
							<input
								id="group-name"
								type="text"
						...
						<div>
							<label htmlFor="group-description" className="text-sm font-medium text-foreground block mb-2">Description</label>
							<textarea
						...
						<div>
							<label htmlFor="group-type" className="text-sm font-medium text-foreground block mb-2">Group Type</label>
							<select
								id="group-type"
								className="w-full px-4 py-2 bg-background border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
						...
						<div className="flex gap-3 pt-4">
							...
							<Button className="flex-1" onClick={handleSubmit} disabled={isLoading || !formData.name}>
								...
								{!isLoading && groupToEdit && "Save Changes"}
								{!isLoading && !groupToEdit && "Create Group"}
							</Button>
						</div>
					</div>
				</Card>
