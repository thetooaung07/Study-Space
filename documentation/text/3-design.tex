...
The domain model consists of the entities that carry the application's persistent state. To ensure clarity, the complete model is divided into four functional subsystems corresponding to the core features described in Chapter~\ref{ch:analysis}. Each subsystem and its entities are described below. The domain model diagrams in this section were produced using DrawDB~\cite{DrawDB}, a browser-based entity-relationship diagram editor. Additionally, a bird's-eye view of all entity relationships is presented in the complete domain model diagram in Appendix~\ref{ch:appendix}, and the schema is provided in the enclosed electronic media as an attached resource.
...
takes one of two values: \texttt{STUDENT} or \texttt{INSTRUCTOR}. The entity also includes
fields for a cumulative study-time counter and a current-streak counter to support future
gamification features. A user can create many \texttt{Course} records (as an Instructor), own many
...
\textbf{Activity} is designed to represent study-related events, carrying a type, an optional text
message, a timestamp, and a reference to the owning user, optionally linked to a specific study session.
...
For local development, all three tiers can be orchestrated using \texttt{docker-\allowbreak compose.yml}~\cite{DockerCompose}
...
The server-side application follows the Model-View-Controller (MVC) layered pattern~\cite{FowlerEAA,RichardsSAP}. The domain model diagrams in this chapter were generated using PlantUML, a plain-text markup tool for UML diagram generation. The \emph{controller layer} receives HTTP requests, delegates to the appropriate service, and serialises the response. The \emph{service layer} contains all business logic, orchestrates data access, and enforces domain rules such as ownership checks and status transitions. The \emph{repository layer} provides data access through named query methods and custom queries for complex lookups. Entity classes map the relational schema to Java objects. Data Transfer Objects (DTOs) define the API contract and decouple the internal entity structure from the shape of the JSON responses, so the two can evolve independently.
...
\begin{table}[!ht]
    ...
    \begin{tabularx}{\textwidth}{|p{3cm}|X|}
        ...
        \hline
        \texttt{mapper} & Stateless mapper classes that convert between entity and DTO
                         representations. \\
        \hline
        ...
        \hline
        \texttt{exception} & Custom exception classes and a global
                            \texttt{@ControllerAdvice} handler that maps them to
                            appropriate HTTP error responses. \\
        \hline
        \texttt{types} & Shared enumerations used across multiple layers, such as
                        \texttt{UserRole}, \texttt{ProposalStatus}, and
                        \texttt{MaterialType}. \\
        \hline
        \texttt{util} & Stateless utility helpers, such as date and time formatting
                       utilities. \\
        \hline
    \end{tabularx}
\end{table}
...
% ----------------------------------------------------------------
\section{User Interface Design and Wireframes}
\label{sec:ui-design}

The design process began by producing low-fidelity wireframes to establish the basic structural layouts and core user flows before initiating frontend development. These initial wireframes represent rough draft layouts, using simple crossed boxes to outline visual element placeholders and minimal outlines to define interface boundaries. The design focused on the most critical interactions of the application: querying the AI assistant (shown in Figure~\ref{fig:wireframe-ai}), submitting a merge proposal (illustrated in Figure~\ref{fig:wireframe-merge-proposal}), and reviewing contribution proposals (depicted in Figure~\ref{fig:wireframe-review}). The wireframes were designed in Figma, a browser-based collaborative interface design tool.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/wireframes/ai_assistant_chat_student_view.png}
    \caption{Low-fidelity wireframe: AI assistant conversational interface}
...
\begin{figure}[H]
    ...
    \caption{Low-fidelity wireframe: Student merge proposal submission}
...
\begin{figure}[H]
    ...
    \caption{Low-fidelity wireframe: Instructor contribution review interface}
