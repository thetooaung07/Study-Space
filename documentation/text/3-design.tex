...
is deployed to a dedicated hosting platform (e.g., Vercel), the backend runs as a containerised
service on a cloud platform (e.g., Render), and file storage is handled by an S3-compatible service
(e.g., Tigris). This decoupling ensures independent scalability and simplifies deployment pipelines.
...
The server-side application follows the Model-View-Controller (MVC) layered pattern~\cite{FowlerEAA,RichardsSAP}. The
...
\textbf{Distributed Object Storage (Tigris)} provides high durability, direct URL serving, and
persistent scaling. Tigris offers an Amazon S3-compatible API, allowing the use of the standard
...
The full API is documented through the Swagger UI available at \texttt{/swagger\allowbreak -ui/\allowbreak index.html}
