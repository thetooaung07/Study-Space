...
the requirements established in the preceding sections. The goal was to select a stack that is
...
\textbf{Vue.js}~\cite{Vuejs} is a progressive framework with a smaller bundle size and a gentler learning
...
\textbf{Angular}~\cite{Angular} is a full-featured framework backed by Google that includes dependency injection,
...
first-class, and its ecosystem provides the resizable panel, rich-text input, and \textbf{WebSocket}~\cite{WebSocketRFC6455} hook
...
optimisation. Compared to a plain Vite~\cite{Vite} or Create React App~\cite{CreateReactApp} setup, Next.js removes boilerplate
...
\textbf{Shadcn/ui}~\cite{ShadcnUI} and \textbf{Radix UI}~\cite{RadixUI}, which provide accessible, unstyled
component primitives that Tailwind classes are applied to. State management across the application
uses React's native \textbf{Context API}~\cite{ReactContextAPI} and custom hooks, providing a lightweight, predictable
...
first-class integration with \textbf{Spring Security}~\cite{SpringSecurity}, \textbf{Spring Data JPA}~\cite{SpringDataJPA}, \textbf{Spring WebSocket}~\cite{SpringWebSocket}, and \textbf{Springdoc OpenAPI}~\cite{SpringdocOpenAPI} - all of which are essential for StudySpace. Virtual Threads, introduced in Java~21,
improve the throughput of concurrent request handling without the complexity of reactive
programming~\cite{VirtualThreadsJEP444}.

\textbf{Node.js}~\cite{Nodejs} with \textbf{NestJS}~\cite{NestJS} or \textbf{Express}~\cite{ExpressJS} excels in I/O-bound tasks due to its event-driven
architecture~\cite{NodejsAbout}. However, its single-threaded event loop can be a bottleneck for the multi-step
...
\textbf{Python}~\cite{Python} with \textbf{Django}~\cite{Django} or \textbf{FastAPI}~\cite{FastAPI} is excellent for rapid prototyping and AI integration.
...
concurrent CPU-bound tasks~\cite{PythonGIL}.
...
and higher-level access is provided by Spring Data JPA~\cite{SpringDataJPA} with Hibernate~\cite{Hibernate} as the ORM provider.

\textbf{MongoDB}~\cite{MongoDB} (NoSQL document store) and \textbf{Cassandra}~\cite{Cassandra} (wide-column store) are optimised
...
denormalisation, duplicating data to avoid application-level joins~\cite{NoSQLDistilled}, which introduces the risk of
...
Additionally, the AI assistant leverages the \texttt{pgvector} extension~\cite{pgvector} to provide similarity
...
\textbf{JSON Web Tokens (JWT)}~\cite{JWT_RFC7519} with Spring Security~\cite{SpringSecurity} enables stateless
...
\textbf{Server-side sessions} with Spring Session~\cite{SpringSession} require a shared session store (such as Redis)
...
\textbf{OAuth~2.0 only}~\cite{OAuth2} (without local credentials) would prevent users without a Google or
...
\textbf{WebSocket}~\cite{WebSocketRFC6455} with \textbf{STOMP}~\cite{STOMP} over \textbf{SockJS}~\cite{SockJS} layers a publish-subscribe model on top
...
\textbf{Server-Sent Events (SSE)}~\cite{SSE} provide a unidirectional push channel from server to client.
...
\textbf{Long Polling}~\cite{LongPollingRFC6202} achieves real-time delivery but requires each client to maintain a pending
...
For StudySpace, the ability to test complex user journeys efficiently is paramount. The parallel execution model of Playwright drastically reduces the time required to run the full E2E suite compared to Cypress~\cite{Jadhav2024, Playwright}. Additionally, its robust handling of modern web APIs and multi-context scenarios ensures that the real-time communication channels and file viewing features can be tested reliably. Consequently, Playwright was chosen as the end-to-end testing framework for the project.
