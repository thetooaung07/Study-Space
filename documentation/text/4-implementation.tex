...
\begin{lstlisting}[language=Java, caption={Code listing 4.4 --- Structured prompt construction (GeminiService.java)}, label=lst:gemini-prompt]
...
\begin{lstlisting}[language=Java, caption={Code listing 4.5 --- JWT validation in the authentication filter (JwtAuthenticationFilter.java)}, label=lst:jwt-filter]
...
\label{subsec:automated-tests}

The project uses three levels of automated tests, all implemented with JUnit~5 and the Spring
Boot Test framework.
