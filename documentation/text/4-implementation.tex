...
The course management interface is shown in Figure~\ref{fig:course-admin}. The page presents the Instructor with a list of sections and a file upload
...
The student Private Workspace interface and its Merge Proposal submission dialog are shown in Figure~\ref{fig:content-extension}. Once the student submits a proposal, the Instructor can review it through the interface illustrated in Figure~\ref{fig:instructor-review}. The workspace view shows the student a mirror of the course structure. An upload button per
...
\begin{figure}[H]
    ...
    \caption{Student Merge Proposal View: Submission dialog from the Private Workspace}
...
The workspace viewer and its integrated contextual chat panel are shown in Figure~\ref{fig:contextual-chat}. The chat panel is shown after a student typed \texttt{@lecture} and the autocomplete dropdown
...
The AI assistant panel is shown in Figure~\ref{fig:ai-assistant}. The panel shows the student's question at the top and the LLM-generated answer below,
...
The student dashboard interface demonstrating the role-restricted layout and navigation options is shown in Figure~\ref{fig:access-control}. The two navigation sidebars illustrate the role distinction. The student sees course browsing
...
\begin{figure}[H]
    ...
    \caption{Student dashboard view illustrating the role-restricted navigation layout}
...
\emph{Unit and integration tests} are executed using Vitest to isolate components such as authentication forms. By mocking the Next.js router and authentication contexts, the tests simulate user input and assert on the resulting DOM updates without relying on a running server.

\emph{End-to-end (E2E) tests} are orchestrated with Playwright to navigate complete user journeys in a headless browser. To verify all five core functional requirements (FR-01 to FR-05) under realistic conditions, a dedicated E2E test suite intercepts API calls and simulates user actions, ensuring that the frontend behaves correctly under various backend states (e.g., successful login or HTTP 401 Unauthorized).
...
guarantee code quality and automate deployment~\cite{ContinuousDelivery}, as shown in Figure~\ref{fig:cicd-pipeline}. The first stages build the backend with Gradle~\cite{Gradle} and
...
SonarQube~\cite{SonarQube}. If all checks pass on the main branch, the final stage allows manual triggering of the deployment jobs. Once triggered, the frontend is deployed to Vercel. A separate pipeline job builds the backend container, pushes it to Docker Hub, and triggers the deployment hook on the Render cloud platform.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{images/uml_diagrams/deployment.png}
    \caption{GitLab CI/CD pipeline stages and deployment workflow}
...
the application is running. Public service methods carry JavaDoc~\cite{Javadoc} comments that explain the
